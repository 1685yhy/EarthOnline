...
namespace EarthOnline
{
    ...
    public class RandomEvents : MonoBehaviour
    {
        ...
        IEnumerator EventLoop()
        {
            ...
            _events.Add(new GameEvent { id = "void_champion", title = "虚空冠军", description = "虚空选中了它的冠军——不是反派，是守护者。", OnTrigger = () => { Debug.Log("[Event] 虚空冠军现身——它不是来杀你的。它说：虚空需要守护者——上一个守护者已经撑了三千年。第47号——你愿意吗？"); PlayerStats.Instance?.AddCultivation(400); } });
            _events.Add(new GameEvent { id = "earth_last", title = "地球最后通信", description = "地球意志发出最后一条信息——信号越来越弱。", OnTrigger = () => { Debug.Log("[Event] 地球意志(信号微弱)：我的能量...不多了...第47号...无论你选择什么...记住——不是你的错..."); PlayerStats.Instance?.AddCultivation(500); } });
            _events.Add(new GameEvent { id = "legacy", title = "传承", description = "一位老穿越者把毕生修为传给了你。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(1000); Debug.Log("[Event] 第1号穿越者把他的修为传给了你——他不去最后的战斗了。他说：我已经活了够久了。让年轻人去吧。"); } });
            _events.Add(new GameEvent { id = "dragons_return", title = "龙族归来", description = "灭绝的龙族——回来了。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(500); Debug.Log("[Event] 龙族归来——它们没有灭绝——它们躲在时间的裂缝里。现在——虚空威胁到了它们。它们选择参战。"); } });
            _events.Add(new GameEvent { id = "immortal_realm", title = "仙界之门", description = "飞升通道短暂开启——仙气涌入。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(800); Debug.Log("[Event] 仙界之门——上一个飞升者打开了通道。她说：下面的人——我们在上面看着你们。不要放弃。"); } });
            _events.Add(new GameEvent { id = "void_offers", title = "虚空的条件", description = "虚空提出了一个交易——停止对抗，它就停止扩张。", OnTrigger = () => { Debug.Log("[Event] 虚空的条件：停止对抗——我就停止扩张。我只想回家。我和你们一样——也是被困在这里的。"); PlayerStats.Instance?.AddCultivation(300); } });
            _events.Add(new GameEvent { id = "hero_sacrifice", title = "英雄的牺牲", description = "一位穿越者为了掩护你——牺牲了自己。", OnTrigger = () => { Debug.Log("[Event] 第5号穿越者——为了掩护你——永远留在了虚空里。他最后的话：替我看看外面的世界——是什么样的。"); PlayerStats.Instance?.AddCultivation(600); } });
            _events.Add(new GameEvent { id = "dawn_war", title = "黎明之战", description = "虚空发动总攻——所有势力联合防御。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(200); Debug.Log("[Event] 黎明之战——天元宗、青云门、散修联盟——第一次并肩作战。虚空不会等——今天是决战。"); } });
            _events.Add(new GameEvent { id = "master_reveal", title = "真相大白", description = "签到系统制造者的身份终于揭露。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(300); Debug.Log("[Event] 签到系统制造者——就是第0号穿越者。比所有人都早——他是第一个。他失败了——但他留下了系统——帮助后来的人。"); } });
            _events.Add(new GameEvent { id = "void_memory", title = "虚空记忆", description = "虚空的记忆涌入你的脑海——你看到了它的过去。", OnTrigger = () => { Debug.Log("[Event] 虚空记忆——它曾经是一个世界。和灵气大陆一样。地球意志创造了它——然后抛弃了它。它的愤怒——不是没有理由的。"); PlayerStats.Instance?.AddCultivation(400); } });
            _events.Add(new GameEvent { id = "last_stand", title = "最后一战", description = "所有穿越者——47人——在此集结。", OnTrigger = () => { Debug.Log("[Event] 47位穿越者——活着的和死去的——都在此刻站在你身后。第1号说：我们准备好了。第46号说：这一次——我们不会失败。"); PlayerStats.Instance?.AddCultivation(1000); } });
            _events.Add(new GameEvent { id = "new_world", title = "新世界", description = "虚空消退后——一片新的大陆浮现。", OnTrigger = () => { Debug.Log("[Event] 新大陆——不是灵气大陆——是一个全新的世界。虚空消退后留下的——一个新的开始。"); PlayerStats.Instance?.AddCultivation(500); } });
            _events.Add(new GameEvent { id = "e77", title = "灵脉共鸣", description = "所有灵脉同时发光——世界在唱歌。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(200); Debug.Log("[Event] 灵脉共鸣——全大陆的灵脉在这一刻同时发光。张老说：三千年了——第一次看到。"); } });
            _events.Add(new GameEvent { id = "e78", title = "古籍解密", description = "一本被封印的古籍——在虚空中被打开了。", OnTrigger = () => { Debug.Log("[Event] 古籍——不是修士写的——是凡人写的。三千年前——一个凡人预言了虚空和穿越者的一切。最后一个预言：第48号——会终结循环。"); PlayerStats.Instance?.AddCultivation(300); } });
            _events.Add(new GameEvent { id = "e79", title = "众神沉默", description = "天道以上——更高的存在——终于开口了。", OnTrigger = () => { Debug.Log("[Event] 众神——不是地球意志——是更高的存在。他们说：我们一直在看。你们的战斗——让我们感动。人类的意志——值得尊敬。"); PlayerStats.Instance?.AddCultivation(500); } });
            _events.Add(new GameEvent { id = "e80", title = "回家", description = "地球意志送你回家——如果你愿意的话。", OnTrigger = () => { Debug.Log("[Event] 地球意志：第47号——你可以回家了。回到地球——回到你穿越之前的那一刻。或者——留在这里。这是你的选择。"); PlayerStats.Instance?.AddCultivation(1000); } });
            _events.Add(new GameEvent { id = "redemption", title = "救赎", description = "猎血人放下武器——他选择了另一条路。", OnTrigger = () => { Debug.Log("[Event] 猎血人跪了下来：我猎杀血脉觉醒者——是为了保护这个世界。现在——让我用剩下的生命来赎罪。"); PlayerStats.Instance?.AddCultivation(300); } });
            _events.Add(new GameEvent { id = "void_queen", title = "虚空女王", description = "虚空的真正统治者——不是王——是女王。", OnTrigger = () => { Debug.Log("[Event] 虚空女王：我是第-1号穿越者。在地球意志之前——我就存在了。虚空是我的孩子——也是我的牢笼。帮我——也是帮你自己。"); PlayerStats.Instance?.AddCultivation(600); } });
            _events.Add(new GameEvent { id = "earth_truth", title = "地球真相", description = "地球意志告诉你——为什么必须是你。", OnTrigger = () => { Debug.Log("[Event] 地球意志：第47号——你知道为什么是你吗？因为你的DNA里——有所有46位穿越者的基因片段。你不是一个人——你是47个人。"); PlayerStats.Instance?.AddCultivation(800); } });
            _events.Add(new GameEvent { id = "final_dawn", title = "最终黎明", description = "无论选择什么——黎明终将到来。", OnTrigger = () => { Debug.Log("[Event] 太阳升起来了——不是灵气大陆的太阳——是地球的太阳。你看到了——家的方向。"); PlayerStats.Instance?.AddCultivation(1000); } });
            _events.Add(new GameEvent { id = "e70", title = "天道崩塌", description = "天道开始崩溃——世界的法则在改变。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(500); Debug.Log("[Event] 天道崩塌——虚空的侵蚀已经触及了世界法则。灵气开始变得不稳定——修士们恐慌了。"); } });
            _events.Add(new GameEvent { id = "e71", title = "凡人崛起", description = "凡人们不再等待修士的保护——他们自己站出来了。", OnTrigger = () => { Debug.Log("[Event] 凡人军队——没有灵力——但他们有勇气。他们用凡铁铸造的武器——虽然伤不了虚空——但他们愿意站在修士前面。"); PlayerStats.Instance?.AddCultivation(200); } });
            _events.Add(new GameEvent { id = "e72", title = "旧敌联手", description = "天元宗宗主和散修联盟盟主——第一次握手。", OnTrigger = () => { Debug.Log("[Event] 天元宗宗主：我做了很多错事——但虚空面前——我们都是人类。散修联盟盟主：暂时休战——打完虚空再算账。"); PlayerStats.Instance?.AddCultivation(300); } });
            _events.Add(new GameEvent { id = "e73", title = "虚空哭泣", description = "虚空——在哭。", OnTrigger = () => { Debug.Log("[Event] 虚空在哭——不是愤怒——是悲伤。它说：我不想吞噬世界——但我控制不了。我不是坏的——我是病了。帮帮我。"); PlayerStats.Instance?.AddCultivation(500); } });
            ...
            _events.Add(new GameEvent { id = "night_4", title = "夜之事件4", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_5", title = "夜之事件5", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_6", title = "夜之事件6", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_7", title = "夜之事件7", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_8", title = "夜之事件8", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_9", title = "夜之事件9", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
            _events.Add(new GameEvent { id = "night_10", title = "夜之事件10", description = "自动生成", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(50); } });
