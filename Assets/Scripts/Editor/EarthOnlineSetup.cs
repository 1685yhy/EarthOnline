...
// BATCH: 7 enemies at once
            ...
            _events.Add(new GameEvent { id = "e81", title = "龙族盟约", description = "龙族正式与人类结盟——共同对抗虚空。", OnTrigger = () => { PlayerStats.Instance?.AddCultivation(300); Debug.Log("[Event] 龙族盟约——龙族之王：从今以后——龙与人类——共同守护这个世界。"); } });
            _events.Add(new GameEvent { id = "e82", title = "古籍藏书阁", description = "天元宗地下发现了一座被封存万年的藏书阁。", OnTrigger = () => { Debug.Log("[Event] 古籍藏书阁——里面记载了3000年前虚空起源的真相——不是天灾——是人祸。"); PlayerStats.Instance?.AddCultivation(400); } });
            _events.Add(new GameEvent { id = "e83", title = "散修封神", description = "一位散修——靠自己的努力——渡劫成功了。", OnTrigger = () => { Debug.Log("[Event] 散修封神——不是宗门弟子——不是世家子弟——一个散修——靠自己——飞升了。他证明了——出身不重要——意志决定一切。"); PlayerStats.Instance?.AddCultivation(300); } });
